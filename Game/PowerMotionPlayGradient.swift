...
struct PowerMotionPlayWebViewBox: UIViewControllerRepresentable {
  // MARK: - Свойства

  @ObservedObject var loader: PowerMotionPlayWebLoader
  var defaultOrientations: UIInterfaceOrientationMask = .portrait

  // MARK: - Координатор

  func makeCoordinator() -> PowerMotionPlayWebCoordinator {
    PowerMotionPlayWebCoordinator { [weak loader] status in
  ...
  func makeUIViewController(context: Context) -> PowerMotionPlayBaseWebViewController {
    let configuration = createWebViewConfiguration()
    return PowerMotionPlayBaseWebViewController(
  ...
  func updateUIViewController(_ uiViewController: PowerMotionPlayBaseWebViewController, context: Context) {
