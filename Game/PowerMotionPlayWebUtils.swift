...
enum PowerMotionPlayWebStatus: Equatable, WebStatusComparable {
  case standby
  ...
  func isEquivalent(to other: PowerMotionPlayWebStatus) -> Bool {
    switch (self, other) {
    case (.standby, .standby),
      (.finished, .finished),
      (.noConnection, .noConnection):
      return true
    case (.progressing(let a), .progressing(let b)):
      return abs(a - b) < 0.0001
    case (.failure(let reasonA), .failure(let reasonB)):
  ...
  var progress: Double? {
    guard case .progressing(let value) = self else { return nil }
    return value
  }
...
extension PowerMotionPlayWebStatus {
...
extension PowerMotionPlayWebStatus {
  static func == (lhs: PowerMotionPlayWebStatus, rhs: PowerMotionPlayWebStatus) -> Bool {
